# -*- coding: utf-8 -*-
"""
Created on Tue Aug 11 10:23:35 2020

@author: Anton
"""

""" 
    KEEP TRACK OF WHAT BOOKS BELONG TO WHICH TOPICS
    BEFORE A BOOK NEEDED TO HAVE THE SAME NAME AS A TOPIC: 
            BOOKS "SQL FOR DUMMIES" -> TOPIC: "SQL FOR DUMMIES"
            
    BUT NOW MULTIPLE PDFS CAN BELONG TO THE SAME TOPIC: 
            BOOKS:"SQL FOR DUMMIES" / "INTRODUCTION TO SQL" / .... -> TOPIC: "SQL"
"""
...
class booktracker(gui.MyFrame):
    def __init__(self,dirsettings):
        
        self.booksfile = Path(dirsettings,"books_and_pdfs.txt")
        if self.booksfile.exists():
            with open(self.booksfile, 'r') as file:
                self.bookdict = json.load(file)
            file.close()
        else:
            self.bookdict = {}
        
    def connect_book_to_topic( self, bookname, topic):
        pass
